                    ...
                    <div class="flex items-center justify-between w-[65%]">
                      ...
                      >
                        ...
                        <Link
                          ...
                          :value="document.doc[field.fieldname]"
                          :doctype="field.fieldtype == 'Link' ? field.options : document.doc[field.options]"
                          :filters="field.filters"
                          :placeholder="field.placeholder"
                          @change="(v) => fieldChange(v, field)"
...
import { Tooltip, DateTimePicker, DatePicker, call } from 'frappe-ui'
...
async function fieldChange(value, df) {
  ...
  try {
    // For address linking, use a single API call to update both the field and create the link
    if (df.fieldtype === 'Link' && df.options === 'Address') {
      const updatedContact = await call('crm.api.contact.update_contact_and_link_address', {
        contact_name: props.docname,
        fieldname: df.fieldname,
        value: value
      })
      // Update the document with the returned data
      document.doc = updatedContact
      // Emit reload event to update parent components
      emit('reload')
    } else {
    ...
    if (error.message?.includes('TimestampMismatchError')) {
      // If we get a timestamp mismatch, reload the document and retry
      await document.reload()
      await fieldChange(value, df)
    } else {
